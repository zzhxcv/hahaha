...
def hello_world2():
    if len(session)>0:
        ...
        if ct>30:
            return render_template("h08_3.html",c=u"登录超时",cc=1)
        return render_template("h08_3.html",c=u"登录成功")
    return render_template("h07_3.html")
@app.route("/1/",methods=["GET","POST"])
def hello_world1():
    h1=request.form.get("user2")
    h2=request.form.get("pw2")
    h3=request.form.get("valid")
    if h3!=None:
